...
export function useAccessScopeOptionsQuery(scopeType:AccessScopeType){
 return useQuery({queryKey:["organization-access-mapping-scope-options",scopeType],queryFn:()=>accessMappingService.scopeOptions(scopeType),enabled:Boolean(scopeType)&&scopeType!=="all"&&scopeType!=="trade_group"});
}
