  ...
  return query<any[]>(
    `SELECT oam.*, r.name AS role_name, s.scope_value
     ...
     INNER JOIN user_roles ur
       ON ur.role_id = oam.role_id
      AND ur.user_id = ?
     INNER JOIN roles r ON r.id = oam.role_id
     INNER JOIN users u ON u.id = ur.user_id
     LEFT JOIN organization_access_mapping_scopes s
       ON s.access_mapping_id = oam.id
     WHERE oam.is_active = 1
       AND (
         (u.team_id IS NOT NULL
           AND oam.team_id = u.team_id
         ...
         OR
         (u.team_id IS NULL AND u.directorate_id IS NOT NULL
           AND oam.team_id IS NULL
           AND oam.directorate_id = u.directorate_id
           AND (oam.department_id IS NULL OR oam.department_id = u.department_id)
           AND (oam.office_id IS NULL OR oam.office_id = u.office_id))
         OR
         (u.team_id IS NULL AND u.directorate_id IS NULL AND u.department_id IS NOT NULL
           AND oam.team_id IS NULL
           AND oam.directorate_id IS NULL
           AND oam.department_id = u.department_id
           AND (oam.office_id IS NULL OR oam.office_id = u.office_id))
         OR
         (u.team_id IS NULL AND u.directorate_id IS NULL AND u.department_id IS NULL AND u.office_id IS NOT NULL
           AND oam.team_id IS NULL
           AND oam.directorate_id IS NULL
           AND oam.department_id IS NULL
           AND oam.office_id = u.office_id)
       )
     ORDER BY oam.id, s.scope_value`,
    [userId],
  );
}
