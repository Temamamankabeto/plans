...
type TradeRecord = {
  ...
  period_type: "annual" | "monthly"; commodity_group: string; commodity: string; unit: string;
...
export default function TradeRecordsPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const [response, settingsResponse, areasResponse, productsResponse] = await Promise.all([
      ...
      <DialogContent className="max-w-4xl bg-white">
        ...
        <form onSubmit={saveAnnual} className="grid gap-4 md:grid-cols-3">
          ...
          <Field label="Product"><Select value={form.commodity} onValueChange={(v)=>setForm({...form,commodity:v})}>
            ...
            <SelectContent className="z-[100] max-h-64 overflow-y-auto bg-white">
              {productOptions.map((x)=><SelectItem key={x.id} value={x.name}>{x.name}</SelectItem>)}
            </SelectContent></Select></Field>
