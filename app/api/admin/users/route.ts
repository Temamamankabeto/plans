...
export async function GET(request: NextRequest) {
  ...
  if (role && role !== "all") {
    where.push("EXISTS (SELECT 1 FROM user_roles ur2 INNER JOIN roles r2 ON r2.id = ur2.role_id WHERE ur2.user_id = u.id AND r2.name = ?)");
    params.push(role);
  }

  const whereSql = where.length ? `WHERE ${where.join(" AND ")}` : "";
  const countRows = await query<any[]>(`SELECT COUNT(*) AS total FROM users u LEFT JOIN offices o ON o.id = u.office_id ${whereSql}`, params);
  const rows = await query<any[]>(`${userSelect} ${whereSql} ${groupBy} ORDER BY u.id DESC LIMIT ? OFFSET ?`, [
    ...params,
    perPage,
    offset,
  ]);

  return paginated(rows.map(formatUser), page, perPage, Number(countRows[0]?.total ?? 0));
}
...
export async function POST(request: NextRequest) {
  ...
  const userId = await transaction(async (conn) => {
    const [insert]: any = await conn.execute(
      `INSERT INTO users (name, email, phone, password, status, office_id, directorate_id, department_id, team_id, professional_level)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        name,
        email,
    ...
    await conn.execute("INSERT INTO user_roles (user_id, role_id) VALUES (?, ?)", [insert.insertId, roleRows[0].id]);
    return insert.insertId;
  });

  const rows = await query<any[]>(`${userSelect} WHERE u.id = ? ${groupBy}`, [userId]);
