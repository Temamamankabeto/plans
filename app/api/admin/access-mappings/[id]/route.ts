...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const duplicate = await query<any[]>(
    `SELECT id FROM organization_access_mappings
     WHERE id<>? AND role_id=? AND module=? AND office_id <=> ? AND department_id <=> ?
       AND directorate_id <=> ? AND team_id <=> ? AND is_active=1 LIMIT 1`,
    [id, roleId, module, organization.office_id, organization.department_id, organization.directorate_id, organization.team_id],
  );
  if (duplicate.length) return fail("This role already has an active mapping for the selected organization and module", 409);

  await transaction(async (connection) => {
    await connection.execute(
      `UPDATE organization_access_mappings SET office_id=?,directorate_id=?,department_id=?,team_id=?,
       role_id=?,module=?,scope_type=?,scope_value=NULL,can_create_annual_plan=?,can_divide_monthly_plan=?,
       can_update_achievement=?,can_view_report=?,can_comment=?,can_approve=?,is_active=? WHERE id=?`,
    ...
    for (const value of resolved.values) {
      await connection.execute("INSERT INTO organization_access_mapping_scopes (access_mapping_id,scope_value) VALUES (?,?)", [id,value]);
    }
  });

  const rows = await query<any[]>(`${mappingSelect} WHERE oam.id=? LIMIT 1`, [id]);
  return ok((await attachScopes(rows))[0] ?? null, "Access mapping updated successfully");
}
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const id = Number(rawId);
  if (!Number.isFinite(id) || id <= 0) return fail("Invalid access mapping id", 422);
  await transaction(async (connection) => { await connection.execute("DELETE FROM organization_access_mappings WHERE id=?", [id]); });
