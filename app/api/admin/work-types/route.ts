...
export async function GET(request: NextRequest) {
  ...
  if (all) {
    ...
    return ok(rows, "Work types fetched successfully");
  }

  const { page, perPage, offset } = pagination(request);
  const countRows = await query<any[]>(`SELECT COUNT(*) AS total FROM work_types ct ${whereSql}`, params);
...
export async function POST(request: NextRequest) {
  ...
  const duplicateRows = await query<any[]>("SELECT id FROM work_types WHERE LOWER(name) = LOWER(?) LIMIT 1", [name]);
  ...
  const result = await execute("INSERT INTO work_types (name, code, is_active) VALUES (?, ?, ?)", [name, code, isActive]);
