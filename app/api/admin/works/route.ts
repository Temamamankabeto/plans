...
export async function POST(r:NextRequest){
 ...
 } else if(!(await query<any[]>("SELECT id FROM livestock_products WHERE id=? AND is_active=1",[d.livestock_product_id])).length)return fail("Selected livestock does not exist or is inactive",422);
